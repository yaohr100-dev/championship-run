...
- Points are allocated to the 10 players proportional to real_PTS^1.2 x overall^0.5 x f_i x position_factor (the ^1.2 exponent gives high scorers a bigger usage share; overall^0.5 lets higher-overall stars score a bit more; no separate bench penalty — low-pts bench are handled by the exponent), rounded so the sum matches the team score. This separates team-level form (affects the total) from individual form (affects the distribution).
