...
- Points are allocated to the 10 players proportional to expectedPts(overall) x (real_pts/expectedPts)^usage_exp x f_i x position_factor, where expectedPts = (overall-55)*0.7 anchors the share to ABILITY (so a 15-ppg role player on an extremely weak team doesn't balloon to 40+ ppg) and the usage ratio is a mild "usage tendency" correction. usage_exp = 0.5 regular season, 0.9 playoffs (stars carry more usage). No separate bench penalty. Rounded so the sum matches the team score. This separates team-level form (affects the total) from individual form (affects the distribution).
