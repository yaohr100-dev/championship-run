...
- Points are allocated to the 10 players proportional to real_PTS^1.5 x overall^0.7 x f_i x position_factor (the exponent gives high scorers a bigger usage share; overall^0.7 lets higher-overall stars score a bit more; no separate bench penalty — low-pts bench are handled by the exponent), rounded so the sum matches the team score. In the playoffs the PTS exponent rises to 1.7, so stars carry more usage (a star's average rises ~+2 ppg while role players cede shots). This separates team-level form (affects the total) from individual form (affects the distribution).
